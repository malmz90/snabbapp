...
export default function AppText({
  ...
  const fontFamily =
    variantFontFamily || FONT_FAMILY[family] || FONT_FAMILY.primary;
  const fontWeight =
    variantStyles?.fontWeight || FONT_WEIGHT[family]?.[weight] || 400;
  ...
  const baseLetterSpacing =
    variantStyles?.letterSpacing || baseSize.letterSpacing;

  const calculatedLineHeight =
    lineHeight === "auto"
      ? baseLineHeight
      : fontSize * (LINE_HEIGHT[lineHeight] || 1.4);

  const calculatedLetterSpacing =
    letterSpacing === "auto" ? baseLetterSpacing : fontSize * letterSpacing;

  const effectiveColor =
    color || variantStyles?.color || COLORS.textPrimary;

  const marginTop =
    spacingTop !== 0 ? setSpacing(spacingTop) : variantStyles?.marginTop;
  const marginBottom =
    spacingBottom !== 0
      ? setSpacing(spacingBottom)
      : variantStyles?.marginBottom;
  const paddingLeft =
  ...
  const textContent = <Component style={textStyle}>{children}</Component>;

  if (marginTop || marginBottom || paddingLeft) {
    return (
      <div style={{ marginTop, marginBottom, paddingLeft }}>
        {textContent}
      </div>
    );
  }
